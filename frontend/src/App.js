...
const App = () => {
    ...
    const [maxFileSizeKB, setMaxFileSizeKB] = useState(null);
    ...
    const handleSubmit = async (e) => {
        ...
        // If format is jpg and max file size is not provided, default to PNG file size
        if (format === 'jpg' && !maxFileSizeKB) {
            const pngMaxFileSizeKB = Math.round(fileSize / 1024);
            formData.append('maxFileSizeKB', pngMaxFileSizeKB);
        } else if (format === 'jpg' && maxFileSizeKB) {
            formData.append('maxFileSizeKB', maxFileSizeKB);
        }
        ...
        <div className="container">
            ...
            <form onSubmit={handleSubmit}>
                ...
                {originalWidth && originalHeight && (
                    <p>Original Pixel Size: {originalWidth} x {originalHeight}</p>
                )}
                {fileSize && (
            ...
            {resizedImage && (
                <div className="resized-image-container">
                    ...
                    <a href={resizedImage} download={getDownloadFilename()}>Download Resized Image</a>
                </div>
            )}
        </div>
    );
